...
// Datos para simular respuesta de Lambda
...
function cargarOpcionesPeliculas() {
  ...
  const peliculas = ['Freelance', 'Película 2', 'Película 3'];
...
function enviarComentario() {
  ...
  const selectPelicula = document.getElementById('pelicula');
  const peliculaSeleccionada = selectPelicula.value;

  const comentario = document.getElementById('comentario').value;

  // Muestra el comentario y la película en la consola (simula la llamada al backend)
  ...
  // Construye la URL con los parámetros (dentro de la función para usar el valor actual)
  const apiGatewayUrl = 'https://ct5hbglvqj.execute-api.us-east-1.amazonaws.com/dev/myresource';
  const apiUrl = `${apiGatewayUrl}?query_canal=${peliculaSeleccionada}`;
  console.log('URL de la API:', apiUrl);

  // Actualiza la lista de comentarios
  actualizarListaComentarios(apiUrl);
}

async function actualizarListaComentarios(apiUrl) {
  try {
      console.log('Realizando API call a:', apiUrl);
